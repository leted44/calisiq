...
export const SCORING_GRID: Record<Progression, ProgressionThresholds> = {
  tuck_planche: {
    ...
    shoulder_protraction: { target: 0.45, tolerance: 0.2 },
    pelvis_deviation: { target: 0, tolerance: 0.15 },
  },
  advanced_tuck_planche: {
    body_line_angle_from_horizontal: { target: 15, tolerance: 8 },
    elbow_angle: { target: 178, tolerance: 8 },
    hip_angle: { target: 110, tolerance: 15 },
    // Recalibré le 2026-08 à partir d'un cas réel jugé 10/10 par Cali League (mesuré 0.621)
  ...
  full_planche: {
    ...
    shoulder_protraction: { target: 0.8, tolerance: 0.2 },
